...
def update(db: Session, sandwich_id: int, sandwich: schemas.SandwichUpdate):
    db_sandwich_query = db.query(models.Sandwich).filter(models.Sandwich.id == sandwich_id)
    update_data = sandwich.model_dump(exclude_unset=True)
    db_sandwich_query.update(update_data, synchronize_session=False)
    db.commit()
    return db_sandwich_query.first()

def delete(db: Session, sandwich_id: int):
    db_sandwich_query = db.query(models.Sandwich).filter(models.Sandwich.id == sandwich_id)
    db_sandwich_query.delete(synchronize_session=False)
    db.commit()
